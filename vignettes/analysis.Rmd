...
The function `add_geocbi()` is highly specific to the underlying form "Burn Grading 0.1".
It translates the form fields to the generic form `calculate_geocbi()`, which
...
```{r}
bg_geocbi %>% reactable_burngrading()
```

## Calibrate dNBR
**Note** This section is pending review and refinement.

* Get Landsat/Sentinel imagery pre and post burn: [ropenSci/getlandsat](https://docs.ropensci.org/getlandsat/)
* Calculate NBR for each pre and post burn scene as $(NIR - SWIR) / (NIR + SWIR)$
...
* Model $GeoCBI = lm(dNBR)$
* Predict modelled GeoCBI for other sites
