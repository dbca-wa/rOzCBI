...
```{r}
bg_data <- ruODK::odata_submission_get(
  verbose = T, 
  tz = "Australia/Perth", 
...
```{r}
data_geocbi <- add_geocbi(bg_data)
```
...
leaflet::leaflet(width = 800, height = 600) %>%
    ...
    leaflet::addAwesomeMarkers(
        ...
        popup = ~ glue::glue(
          ...
          "<div>{no_standing_trees_with_hollows} standing trees with hollows</div>",
...
```{r}
bg_data %>% reactable::reactable(sortable = T, filterable = T)
```
